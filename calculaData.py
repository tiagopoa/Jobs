...
#Separa a data em partes, dia, mes, ano, horas e minutos.
    day=int(date[0:2])
    month=int(date[3:5])
    year=int(date[6:10])
    hour=int(date[11:13])
...
#Descobre quantos dias, horas e minutos precisam ser incrementados ou decrementados.
...
#Exibe a quantidade de tempo que precisa ser incrementada ou decrementada.
...
#Quantidade máxima de dias que o mês inicial possui.
#Controla a quantidade de dias a ser decrementada pela variavel days_left.
#Soma os dias na variavel day e verificando se o mês tem 28, 30 ou 31 dias.
...
#Mas se a variavel month já possuir o valor 12, então a variável month recebe o valor 1 e então incrementa a variavel year em 1.
    ...
    while days_left>0:
        ...
        if op=='+':
            if day<max_month_days and days_left>0:
                day+=1
                days_left-=1
            else:
                day=1
...
#Austa as horas e minutos.
            if (hour+hours)>24 and days_left==0:
                hour=(hour+hours)%24
                day+=1

#Calculo para decrementar dias, horas e minutos.
        if op=='-':
...
#Ajusta as horas e minutos.
    ...
    print()
#Exibe o resultado da data inicial + ou - a quantidade em minutos, dependendo da opção escolhida (+ ou -).
